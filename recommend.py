...
def group_by_centroid(restaurants, centroids):
    ...
    """
    # BEGIN Question 4
    close = [[find_closest(restaurant_location(r), centroids), r] for r in restaurants]
...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    lat = mean([restaurant_location(r)[0] for r in cluster])
    long = mean([restaurant_location(r)[1] for r in cluster])
    return [lat, long]
    # END Question 5
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        # BEGIN Question 6
        list_cluster = group_by_centroid(restaurants, centroids)
        centroids = [find_centroid(r) for r in list_cluster]
        # END Question 6
        n += 1
    return centroids
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    f = lambda x: find_predictor(user, reviewed, x)[1]
    fn = max(feature_fns, key= f)
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        if restaurant in reviewed:
            diction[restaurant_name(restaurant)] = user_rating(user, restaurant_name(restaurant))
        else:
            diction[restaurant_name(restaurant)] = predictor(restaurant)
    return diction
    # END Question 9
